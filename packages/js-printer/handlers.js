// https://github.com/Rich-Harris/code-red/blob/6e2a690e4303700de88abc7e6a5b75c318f9108d/src/print/handlers.js
...
const handle_var_declaration = (node, state) => {
	...
	const multiple_lines = declarators.some(has_newline) ||
...
const handlers = {
	...
	ReturnStatement (node, state) {
		if (node.argument) {
			const contains_comment = node.argument.leadingComments &&
	...
	FunctionDeclaration (/** @type {FunctionDeclaration} */ node, state) {
		...
		const multiple_lines = params.some(has_newline) ||
	...
	ArrayExpression (node, state) {
		...
		const multiple_lines = elements.some(has_newline) ||
	...
	ConditionalExpression (node, state) {
		...
		const multiple_lines = has_newline(consequent) ||
	...
	Literal (/** @type {Literal} */ node, state) {
		if (typeof node.value === 'string') {
			return [c(node.raw || JSON.stringify(node.value), node)];
		}
